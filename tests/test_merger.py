...
class TestSessionMerger:
    ...
    def test_process_only_session(self) -> None:
        ...
        processes = [ProcessInfo(pid=100, cpu_percent=20.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj", command="claude")]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
        assert len(merged) == 1
        m = merged[0]
    ...
    def test_process_only_low_cpu_is_idle(self) -> None:
        states: list[SessionState] = []
        processes = [ProcessInfo(pid=100, cpu_percent=1.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj", command="claude")]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
    ...
    def test_matched_by_cwd(self) -> None:
        ...
        processes = [ProcessInfo(pid=100, cpu_percent=5.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj", command="claude")]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
        assert len(merged) == 1
        m = merged[0]
    ...
    def test_cpu_override_idle_to_thinking(self) -> None:
        ...
        processes = [ProcessInfo(pid=100, cpu_percent=25.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj", command="claude")]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
        assert merged[0].effective_status == SessionStatus.THINKING
    ...
    def test_not_stale_with_process(self) -> None:
        ...
        processes = [ProcessInfo(pid=100, cpu_percent=0.0, memory_mb=0.0, elapsed_seconds=600, cwd="/tmp/proj", command="claude")]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
    ...
    def test_multiple_unmatched_processes_same_cwd_all_kept(self) -> None:
        ...
        states: list[SessionState] = []
        processes = [
            ProcessInfo(pid=100, cpu_percent=5.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj", command="claude"),
            ProcessInfo(pid=101, cpu_percent=25.0, memory_mb=0.0, elapsed_seconds=120, cwd="/tmp/proj", command="claude"),
            ProcessInfo(pid=102, cpu_percent=1.0, memory_mb=0.0, elapsed_seconds=30, cwd="/tmp/proj", command="claude"),
        ]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
    ...
    def test_dedup_keeps_different_cwds_separate(self) -> None:
        ...
        states: list[SessionState] = []
        processes = [
            ProcessInfo(pid=100, cpu_percent=5.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj-a", command="claude"),
            ProcessInfo(pid=101, cpu_percent=25.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj-b", command="claude"),
        ]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
        assert len(merged) == 2
    ...
    def test_dedup_skips_empty_cwd(self) -> None:
        ...
        states: list[SessionState] = []
        processes = [
            ProcessInfo(pid=100, cpu_percent=5.0, memory_mb=0.0, elapsed_seconds=60, cwd="", command="claude"),
        ]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
        assert len(merged) == 0
    ...
    def test_hysteresis_stays_thinking_when_cpu_drops_slightly(self) -> None:
        ...
        procs_high = [ProcessInfo(pid=100, cpu_percent=15.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj", command="claude")]
        merged = merger.merge(states, procs_high)
        assert merged[0].effective_status == SessionStatus.THINKING

        # Second merge: CPU dropped to 7% (above drop threshold of 3%) → stays THINKING
        procs_mid = [ProcessInfo(pid=100, cpu_percent=7.0, memory_mb=0.0, elapsed_seconds=62, cwd="/tmp/proj", command="claude")]
        merged = merger.merge(states, procs_mid)
        assert merged[0].effective_status == SessionStatus.THINKING

        # Third merge: CPU drops below 3% → back to IDLE
        procs_low = [ProcessInfo(pid=100, cpu_percent=2.0, memory_mb=0.0, elapsed_seconds=64, cwd="/tmp/proj", command="claude")]
    ...
    def test_time_hold_prevents_idle_flicker_between_tools(self) -> None:
        ...
        procs = [ProcessInfo(pid=100, cpu_percent=1.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj", command="claude")]
    ...
    def test_hysteresis_resets_for_new_sessions(self) -> None:
        ...
        procs = [ProcessInfo(pid=100, cpu_percent=5.0, memory_mb=0.0, elapsed_seconds=60, cwd="/tmp/proj", command="claude")]
    ...
    def test_hookless_process_never_stale(self) -> None:
        ...
        processes = [ProcessInfo(
            pid=100, cpu_percent=0.0, memory_mb=0.0, elapsed_seconds=3700,  # >1h
    ...
    def test_hookless_process_not_stale_when_active(self) -> None:
        ...
        states: list[SessionState] = []
        processes = [ProcessInfo(
            pid=100, cpu_percent=15.0, memory_mb=0.0, elapsed_seconds=90000,  # 25h but active
            cwd="/tmp/proj", command="claude",
        )]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
    ...
    def test_hookless_process_not_stale_when_young(self) -> None:
        ...
        states: list[SessionState] = []
        processes = [ProcessInfo(
            pid=100, cpu_percent=0.0, memory_mb=0.0, elapsed_seconds=600,  # 10 min
            cwd="/tmp/proj", command="claude",
        )]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
    ...
    def test_match_hook_subdirectory_of_process_cwd(self) -> None:
        ...
        processes = [ProcessInfo(
            pid=100, cpu_percent=15.0, memory_mb=0.0, elapsed_seconds=60,
    ...
    def test_match_process_subdirectory_of_hook_cwd(self) -> None:
        ...
        processes = [ProcessInfo(
            pid=200, cpu_percent=10.0, memory_mb=0.0, elapsed_seconds=60,
            cwd="/Users/testuser/projects/bbmedia/projects/podcast",
            command="claude",
        )]
        merger = SessionMerger()
    ...
    def test_no_match_unrelated_paths(self) -> None:
        ...
        processes = [ProcessInfo(
            pid=300, cpu_percent=5.0, memory_mb=0.0, elapsed_seconds=60,
    ...
    def test_multiple_sessions(self) -> None:
        ...
        processes = [
            ProcessInfo(pid=1, cpu_percent=20.0, memory_mb=0.0, elapsed_seconds=60, cwd="/a", command="claude"),
            ProcessInfo(pid=2, cpu_percent=0.0, memory_mb=0.0, elapsed_seconds=60, cwd="/c", command="claude"),
        ]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
...
class TestSessionMergerDedup:
    ...
    def test_same_cwd_keeps_most_recent(self) -> None:
        ...
        processes = [
            ProcessInfo(pid=100, cpu_percent=20.0, memory_mb=0.0, elapsed_seconds=30, cwd="/proj", command="claude"),
        ]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
    ...
    def test_same_cwd_both_have_processes_keeps_newest(self) -> None:
        ...
        processes = [
            ProcessInfo(pid=100, cpu_percent=20.0, memory_mb=0.0, elapsed_seconds=30, cwd="/proj", command="claude"),
            ProcessInfo(pid=200, cpu_percent=5.0, memory_mb=0.0, elapsed_seconds=60, cwd="/proj", command="claude"),
        ]
        merger = SessionMerger()
        merged = merger.merge(states, processes)
        non_stale = [m for m in merged if not m.is_stale]
...
class TestSessionMergerUUIDMatching:
    ...
    def test_parent_child_dirs_no_cross_match(self) -> None:
        ...
        processes = [
            ProcessInfo(
                pid=100, cpu_percent=5.0, memory_mb=0.0, elapsed_seconds=600,
                cwd="/path/BOMBO",
                command="claude",
                session_uuid="parent-uuid",
            ),
            ProcessInfo(
                pid=200, cpu_percent=15.0, memory_mb=0.0, elapsed_seconds=30,
                cwd="/path/BOMBO/bombo_payments",
                command="claude",
